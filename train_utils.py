...
def train_with_validation(model, criterion, optimizers, scheduler, dataloaders, num_epochs, trial, cycle, out_dir, display_every =10, early_stop_tolerance=100, la=None):
    print('>> Train a Model.')
    ...
    for epoch in tqdm(range(num_epochs), leave=False):
        ...
        t_loss = train_epoch(model, criterion, optimizers, dataloaders,scaler=scaler, la=la)
        scheduler.step()
...
def train_epoch(model, criterion, optimizer, dataloaders, vis=None, plot_data=None, scaler=None, la=None):
    model.train()
    # global iters
    iters = 0
    total_loss = 0
    ...
    for data in dataloaders['train']:
        ...
        with torch.cuda.amp.autocast():
            ...
            # TODO: check the criteria, if it does not produce average loss, perform averaging here
            # loss = target_loss
...
def train_with_kd(model, criterion, optimizers, scheduler, dataloaders, num_epochs, trial, cycle, out_dir, display_every = 2, early_stop_tolerance=100, temp=1, alpha=1):
    print('>> Train a Model.')
    # checkpoint_dir = os.path.join('./cifar10', 'train', 'weights')
    # if not os.path.exists(checkpoint_dir):
    #     os.makedirs(checkpoint_dir)
...
def train_epoch_kd(model, criterion, optimizer, dataloaders, temperature, alpha, vis=None, plot_data=None):
    ...
    for data in dataloaders['train']:
        ...
        target_loss = torch.nn.CrossEntropyLoss()(output, target_soft)
...
def train_with_selfkd(model, teacher_model, criterion, optimizers, scheduler, dataloaders, num_epochs, trial, cycle, out_dir, display_every = 10, early_stop_tolerance=100, temp=1, alpha=1):
    print('>> Train a Model.')
    # checkpoint_dir = os.path.join('./cifar10', 'train', 'weights')
    # if not os.path.exists(checkpoint_dir):
    #     os.makedirs(checkpoint_dir)
    ...
    for epoch in tqdm(range(num_epochs), leave=False):
        ...
        t_loss = train_epoch_selfkd(model, teacher_model, criterion, optimizers, dataloaders, temp, alpha)
        scheduler.step()
...
def train_epoch_selfkd(model, teacher_model, criterion, optimizer, dataloaders, temperature, alpha, vis=None, plot_data=None):
    model.train()
    # global iters
    iters = 0
    total_loss = 0
    ...
    for data in dataloaders['train']:
        ...
        # obtain soft labels from teacher model
        teacher_model.eval()
        with torch.no_grad():
            target_teacher = teacher_model(input)

        optimizer.zero_grad()
        logits = model(input)
        
        # loss_ce = criterion(logits, target_hard)
        # loss_ce = torch.sum(loss_ce) / loss_ce.size(0)
        # loss_kd = torch.nn.KLDivLoss(reduction='sum')(F.log_softmax(logits / temperature, dim=1), F.softmax(target_soft, dim=1))
        # target_loss = loss_ce + loss_kd
        
        loss_ce = torch.nn.CrossEntropyLoss()(logits, target)
        loss_kd = torch.nn.KLDivLoss(reduction='batchmean')(F.log_softmax(logits / temperature, dim=1), 
                                                            F.softmax(target_teacher / temperature, dim=1))
        # loss_kd = torch.nn.KLDivLoss(reduction='batchmean')(F.log_softmax(logits, dim=1), 
                                                            # F.softmax(target_teacher / temperature, dim=1))
        loss = (1. - alpha) * loss_ce + (alpha * temperature * temperature) * loss_kd
